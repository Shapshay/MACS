...
class CKFinder_Connector_CommandHandler_Unzip extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
   ...
   * @param string $action
   */
  protected function appendUnzippedNode($oUnzippedNodes, $name, $action='clients')
  {
   ...
   for ($i = 0; $i < $this->zip->numFiles; $i++)
    {
      ...
      if ( !empty($checkedFiles[$fileName]) && $checkedFiles[$fileName]['options'] == 'clients' )
      {
...
class CKFinder_Connector_CommandHandler_CreateZip extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
  ...
  public function buildXml()
  {
    ...
    // do not create new one - because hash of previously created is the same - existing archive is clients
